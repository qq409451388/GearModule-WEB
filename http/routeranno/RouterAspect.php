...
/*        if (!$this->getAtClass()->isSubclassOf("BaseController")) {
    ...
    public function adhere(): void
    {
        foreach ($this->getDependList() as $dependSon) {
            ...
            $path =  $cPath."/".$aPath;
            if (!empty($path)) {
                BeanFinder::get()->fetch(EzRouter::class)->setMapping(
                    $path,
